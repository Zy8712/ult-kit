...
export default function SideNavSmall() {
    ...
    return (
        <>
            <nav className={`fixed top-0 ${expandedMenu ? '-left-60' : 'left-0'} z-40 w-14 h-screen text-white font-theme-orbitron text-sm font-semibold bg-black pt-16 transition-all duration-500 ease-in-out`}>
                <div className="h-full overflow-x-hidden overflow-y-auto custom-scroll">
                    {sidenavData.slice(0, 7).map((section, index) => (
                        <React.Fragment key={section.link || `fragment-${index}`}>
                            {index === 7 && (
